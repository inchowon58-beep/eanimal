...
export function buildPlacesListSeoCopy(opts: {
  ...
  const paragraphs = [
    `${SITE.name}는 ${region} 지역의 ${cat} 인허가 현황을 정리해 안내합니다. 반려동물과 함께 생활하는 가구가 늘어나면서 병원·약국·장묘 시설을 미리 파악하는 일은 일상 돌봄과 응급 대응 모두에서 중요합니다. 본 페이지는 네이버·구글 등 검색 유입을 고려해 서버에서 HTML을 구성하며, 화면에는 필터 조건과 시설 목록, 지역 맥락 설명이 텍스트로 포함됩니다.`,
    ...
    `${cat} 카테고리별로 이용하는 목적이 다릅니다. 동물병원은 진료·접종·응급 처치의 거점이고, 동물약국은 처방·상담을 보완하는 접근점이며, 동물장묘업은 반려동물 장례·화장·봉안 서비스를 담당합니다. ${region} 거주자라면 집·직장 동선과 야간·주말 이용 가능성을 함께 검토하면 실제 선택 폭이 넓어집니다. ${totalText}`,
    ...
    `데이터 해석 시 유의사항이 있습니다. 인허가상 "영업중"이더라도 임시 휴무·이전·상호 변경이 있을 수 있고, 폐업·휴업 표시는 동기화 시점 기준으로 늦을 수 있습니다. 따라서 방문·예약 전에는 해당 시설에 직접 연락해 운영 여부를 재확인하세요. ${SITE.name}는 정보를 중개·안내하는 포털이며, 개별 시설의 서비스 품질을 보증하지 않습니다.`,
...
export function buildPlaceDetailSeoCopy(place: Place): string {
  ...
  const paragraphs = [
    ...
    `${place.category} 이용 목적에 따라 준비물이 달라집니다. 진료·처방·장묘 예약이 필요할 수 있고, 응급 상황에서는 인근 대체 시설을 함께 알아두는 것이 좋습니다. ${SITE.name} 목록에서 같은 ${place.sido || "시·도"}·${place.sigungu || "시·군·구"} 조건으로 유사 카테고리를 비교하면 선택지가 늘어납니다. 상태가 폐업·휴업으로 바뀐 경우 동기화 시각(updated_at: ${place.updated_at}) 이후 변경일 수 있으니 방문 전 전화를 권장합니다.`,
    ...
    `본 정보는 공공 인허가 데이터를 바탕으로 한 안내이며, 의료·약사·장묘 계약의 법적 효력을 대신하지 않습니다. 실제 서비스 가능 여부, 비용, 운영 시간은 ${place.title}에 문의하세요. ${SITE.name}는 ${region} 반려동물 생활 인프라를 더 투명하게 연결하는 것을 목표로 합니다.`,
  ];
