...
export async function getFeaturedPartners(): Promise<FeaturedPartner[]> {
  return readJson<FeaturedPartner[]>("featured-partners.json", []);
}

export async function saveFeaturedPartners(partners: FeaturedPartner[]): Promise<void> {
  await writeJson("featured-partners.json", partners);
}

export async function addFeaturedPartner(partner: FeaturedPartner): Promise<void> {
  const partners = await getFeaturedPartners();
  partners.unshift(partner);
  await saveFeaturedPartners(partners);
}

export async function deleteFeaturedPartner(id: string): Promise<void> {
  const partners = await getFeaturedPartners();
  await saveFeaturedPartners(partners.filter((p) => p.id !== id));
}

export async function updateFeaturedPartner(
  id: string,
  updates: Partial<FeaturedPartner>
): Promise<FeaturedPartner | null> {
  const partners = await getFeaturedPartners();
